...
#     # 正则表达式用于匹配尺寸部分，直到遇到 N= 或其他非尺寸信息
#     size_pattern = re.compile(r"(\d+[*xX]\d+)(?= N=|\s*N=|$)", re.IGNORECASE)
#     match = size_pattern.search(model)
#     return match.group(1) if match else model.strip()
...
def parse_description(description):
    ...
    model_pattern = re.compile(r"2、规格、压力等级\s*：\s*(.*?)(?=3、|$)", re.IGNORECASE)
...
def process_excel_files(file_path_a, file_path_b, output_file_path):
    ...
    for _, row in df_b.iterrows():
        ...
        # 只添加有效的记录到映射中
        if material_name == "金属管壳式消声器" and model and pd.notna(contract_quantity):
            contract_quantity_map[(material_name, extract_non_chinese(model))] = contract_quantity
            # print(f"添加到映射: 名称={material_name}, 型号={model}, 合同数量={contract_quantity}")
...
if __name__ == "__main__":
    ...
    output_file_path = Path("数据19.xlsx")
